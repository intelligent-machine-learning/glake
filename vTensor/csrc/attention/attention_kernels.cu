...
#pragma unroll
  ...
  cache_t* seq_k_ptr = *(k_cache + slot_mapping[seq_idx]);
...
#pragma unroll
      for (int j = 0; j < NUM_VECS_PER_THREAD; j++) {
        const cache_t* k_ptr = seq_k_ptr + block_idx * kv_block_stride
...
#pragma unroll
  ...
  scalar_t zero_value;
  zero(zero_value);
  cache_t* seq_v_ptr = *(v_cache + slot_mapping[seq_idx]);
  for (int block_idx = start_block_idx + warp_idx; block_idx < end_block_idx; block_idx += NUM_WARPS) {
    // NOTE(woosuk): The block number is stored in int32. However, we cast it to int64
    // because int32 can lead to overflow when this variable is multiplied by large numbers
    // (e.g., kv_block_stride).
    ...
    const cache_t* v_ptr = seq_v_ptr + block_idx * kv_block_stride
                                   + kv_head_idx * kv_head_stride;
#pragma unroll
    for (int i = 0; i < NUM_ROWS_PER_THREAD; i++) {
...
__global__ void paged_attention_v1_kernel(
  ...
  paged_attention_kernel<scalar_t, cache_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, KV_DTYPE>(
    ...
    out, q, k_cache, v_cache, num_kv_heads, scale, seq_lens,
    alibi_slopes, q_stride, kv_block_stride, kv_head_stride, kv_scale, slot_mapping);
}
...
void paged_attention_v1_launcher(
  torch::Tensor& out,
  torch::Tensor& query,
  CUdeviceptr key_cache,
  CUdeviceptr value_cache,
  int num_kv_heads,
  float scale,
  ...
  int kv_block_stride = BLOCK_SIZE * num_kv_heads * head_size;
  int kv_head_stride = BLOCK_SIZE * head_size;
  ...
  long* slot_mapping_ptr = slot_mapping.data_ptr<long>();
...
#define CALL_V1_LAUNCHER_BLOCK_SIZE(T, CACHE_T, KV_DTYPE)         \
  switch (block_size) {                                           \
    case 1:                                                       \
      CALL_V1_LAUNCHER(T, CACHE_T, 1, KV_DTYPE);                  \
      break;                                                      \
    case 16:                                                      \
...
void paged_attention_v1(
  ...
  CUdeviceptr key_cache,       // [num_blocks, num_heads, head_size/x, block_size, x]
  CUdeviceptr value_cache,     // [num_blocks, num_heads, head_size, block_size]
